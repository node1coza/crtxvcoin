...
contains(USE_UPNP, -) {
    ...
    DEFINES += USE_UPNP=$$USE_UPNP STATICLIB DMINIUPNP_STATICLIB
...
QMAKE_CXXFLAGS_WARN_ON = -fdiagnostics-show-option -Wall -Wextra -Wformat -Wformat-security -Wno-unused-parameter -fexceptions -Wstack-protector
...
isEmpty(BOOST_LIB_PATH) {
    macx:BOOST_LIB_PATH = /usr/local/opt/boost@1.66/lib
}

isEmpty(BOOST_INCLUDE_PATH) {
    macx:BOOST_INCLUDE_PATH = /usr/local/opt/boost@1.66/include
}
...
isEmpty(MINIUPNPC_INCLUDE_PATH) {
    macx:MINIUPNPC_INCLUDE_PATH = /usr/local/include/miniupnpc
}
...
isEmpty(LIBPNG_INCLUDE_PATH) {
    macx:LIBPNG_INCLUDE_PATH = /usr/local/include/libpng16
}

isEmpty(LIBPNG_LIB_PATH) {
    macx:LIBPNG_LIB_PATH = /usr/local/lib
}
